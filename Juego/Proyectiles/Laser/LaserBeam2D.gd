...
onready var audio_laser_SFX := $AudioStreamPlayer2D
...
func set_is_casting(cast: bool) -> void:
	...
	if is_casting:
		audio_laser_SFX.play()
		cast_to = Vector2.ZERO
		fill.points[1] = cast_to
		appear()
	else:
		audio_laser_SFX.stop()
...
func cast_beam(delta: float) -> void:
	...
	if is_colliding():
		...
		if self.get_collider().has_method("recibir_danio"):
			self.get_collider().recibir_danio(radio_danio * delta)
